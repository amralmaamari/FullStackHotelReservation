...
namespace HotelDataAccessLayer
{
    ...
    public class clsBookingData
    {
        ...
        public static BookingDTOS.BookingDetailsDTO GetBookingDetailsByID(int BookingID)
        {
            using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
            {
                connection.Open();
                ...
                try
                {
                    using (SqlCommand command = new SqlCommand("SP_GetBookingDetailsByID", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@BookingID", BookingID);
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                string roomsJson = reader["Rooms"].ToString();

                                var rooms = JsonSerializer.Deserialize<List<BookingDTOS.RoomDTO>>(roomsJson);

                                return new BookingDTOS.BookingDetailsDTO
                                {
                                    BookingID = (int)reader["BookingID"],
                                    CheckIn = (DateTime)reader["CheckIn"],
                                    CheckOut = (DateTime)reader["CheckOut"],
        ...
        public static List<BookingDTOS.BookingDetailsDTO> GetBookingDetailsByUserID(int UserID)
        {
            var result = new List<BookingDTOS.BookingDetailsDTO>();
            ...
            using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
            {
                ...
                try
                {
                    using (SqlCommand command = new SqlCommand("SP_GetBookingDetailsByUserID", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                // ???? ???? BookingDetailsDTO ??????
                                var dto = new BookingDTOS.BookingDetailsDTO
                                {
                                    BookingID = (int)reader["BookingID"],
                                    CheckIn = (DateTime)reader["CheckIn"],
                                    CheckOut = (DateTime)reader["CheckOut"],
                                    ...
                                    CreatedAt = (DateTime)reader["CreatedAt"],
                                    UpdatedAt = (DateTime)reader["UpdatedAt"],
                                    Rooms = rooms ?? new List<BookingDTOS.RoomDTO>()
                                };
